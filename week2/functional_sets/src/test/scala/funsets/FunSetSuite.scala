...
class FunSetSuite extends FunSuite {
  ...
  test("intersect contains only elements in common") {
    new TestSets {
      val u = union(s1, s2)
  ...
  test("diff contains only elements in first that are not in second") {
    new TestSets {
      val u = union(s1, s2)
      val d = diff(u, s2)
      assert(contains(d, 1))
  ...
  test("filter contains only filtered elements") {
    new TestSets {
      val zeroToFour = union(union(union(s1, s2), s3), s4)
      val even = filter(zeroToFour, (x: Int) => x % 2 == 0)
  ...
  test("forall all even") {
    new TestSets {
      val allEven = union(union(s2, s4), s6)
      assert(forall(allEven, (x: Int) => x % 2 == 0))
    }
  }

  test("forall not all even") {
    new TestSets {
      val notAllEven = union(union(s2, s4), s3)
